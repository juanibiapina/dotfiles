...
- **Description** — write a trigger-oriented description for the frontmatter. Include keywords and phrases that help the agent recognize when to load this skill (e.g., "Use when asked to...", "Triggers on..."). This is how the agent discovers the skill — make it count.
...
- **Global** (`dotfiles/agents/.agents/skills/<name>/SKILL.md` in the dotfiles repo) — use for capabilities useful across all projects. No `make` needed — the agents directory is symlinked by Nix.

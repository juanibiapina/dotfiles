...
`gws gmail users labels list --params '{"userId": "me"}' --format json` returns a JSON **object** with a `labels` key — not a bare array:

```json
{"labels": [{"id": "Label_123", "name": "House Search", "type": "user"}, ...]}
```

Extract a label ID by name with jq:

```bash
gws gmail users labels list --params '{"userId": "me"}' --format json \
  | jq -r '.labels[] | select(.name == "NAME") | .id'
```

Or in Python (note `['labels']`):

```bash
gws gmail users labels list --params '{"userId": "me"}' --format json \
  | python3 -c "import json,sys; labels=json.load(sys.stdin)['labels']; print(next(l['id'] for l in labels if l['name'] == 'NAME'))"
```

Do **not** iterate `json.load(sys.stdin)` directly — that iterates the dict keys, not the label objects, causing `TypeError: string indices must be integers`.
